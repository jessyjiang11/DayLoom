...
export function ItemEditor({ open, item, parentId = null, onClose, onSave }: Props) {
  ...
  async function submit(event: FormEvent) {
    ...
    setError(null)
    try {
      await onSave({ parent_id: item?.parent_id ?? parentId, title: title.trim(), description, kind, status, ...schedule })
      onClose()
    } catch {
    ...
    <Dialog open={open} title={item ? '编辑计划项' : '添加计划项'} onClose={onClose} footer={<><Button variant="secondary" type="button" onClick={onClose}>取消</Button><Button form="item-editor-form" type="submit" loading={saving}>保存</Button></>}>
      <form id="item-editor-form" className="item-editor-form" onSubmit={submit}>
        ...
        <div className="item-editor__pair">
          <label className="ui-field"><span className="ui-field__label">角色</span><select aria-label="角色" className="ui-field__control" value={kind} onChange={(event) => setKind(event.target.value as PlanItem['kind'])}><option value="direction">人生方向</option><option value="goal">阶段目标</option><option value="project">项目</option><option value="task">普通任务</option></select></label>
        ...
        <fieldset className="schedule-fieldset"><legend>执行排期</legend><p>目标归属与时间安排彼此独立</p><div className="schedule-options">{[['', '暂不安排'], ['month', '某个月'], ['week', '某一周'], ['day', '某一天'], ['time', '具体时间']].map(([value, label]) => <label key={label}><input type="radio" name="schedule" checked={(granularity ?? '') === value} onChange={() => setGranularity((value || null) as PlanItem['schedule_granularity'])} />{label}</label>)}</div>
