  ...
  function branch(item: PlanItem, depth = 0) {
    ...
    return <li key={item.id}>
      <div className={`goal-node ${selectedId === item.id ? 'is-selected' : ''}`} style={{ '--depth': depth } as React.CSSProperties}>
        ...
        <button className="goal-node__main" onClick={() => onSelect(item)}><strong>{item.title}</strong><small>{roleLabel[item.kind]} · {statusLabel[item.status]} · {formatScheduleLabel(item)}</small></button>
