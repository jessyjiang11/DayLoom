...
function HomeGoalCards({ goals, items }: { goals: PlanItem[]; items: PlanItem[] }) {
  if (!goals.length) return null
  return <section className="home-goals"><header><strong>一直想记得的方向</strong><Link to="/goals">查看目标树</Link></header><div>{goals.map((goal) => {
    const children = items.filter((item) => item.parent_id === goal.id)
    const done = children.filter((item) => item.status === 'done').length
    const progress = children.length ? Math.round(done / children.length * 100) : 0
    const next = children.find((item) => item.status !== 'done')
    return <article key={goal.id}><div><small>{goal.is_actionable ? '执行目标' : '长期目标'}</small><strong>{goal.title}</strong></div><div className="home-goal__progress"><span style={{ width: `${progress}%` }} /></div><p>{next ? `下一项：${next.title}` : children.length ? '子计划已经全部完成' : '还没有添加子计划'}</p></article>
  })}</div></section>
}
...
export function TodayPage() {
  ...
  const todayItems = items.filter((item) => item.is_actionable && item.schedule_date === today)
  const focus = items.filter((item) => item.is_actionable && item.is_focus && item.status !== 'done').slice(0, 3)
  const homeGoals = items.filter((item) => item.show_on_home && item.status !== 'done')
  ...
  const inbox = items.filter((item) => item.is_actionable && !item.parent_id && !item.schedule_granularity && item.kind === 'task')
