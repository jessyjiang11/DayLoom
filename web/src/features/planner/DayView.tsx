...
export function DayView({ referenceDate, items }: { referenceDate: string; items: PlanItem[] }) {
  ...
  return <div className="day-view"><DropZone id={`day:${referenceDate}`} className="day-board" label="当天未定时间计划"><header><strong>{format(parseISO(referenceDate), 'M月d日 EEEE', { locale: zhCN })}</strong><small>未定时间</small></header><div className="day-untimed">{untimed.map((item) => <TaskChip key={item.id} item={item} />)}</div><div className="hour-list">{Array.from({ length: 13 }, (_, i) => i + 8).map((hour) => <div className="hour-row" key={hour}><time>{String(hour).padStart(2, '0')}:00</time><div>{timed.filter((item) => Number(item.schedule_start_time?.slice(0, 2)) === hour).map((item) => <TaskChip key={item.id} item={item} />)}</div></div>)}</div></DropZone></div>
}
