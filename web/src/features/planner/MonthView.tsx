...
export function MonthView({ referenceDate, items, onAdd, onAddToMonth }: { referenceDate: string; items: PlanItem[]; onAdd?: (date: string) => void; onAddToMonth?: (date: string) => void }) {
  ...
  return <div className="month-view"><DropZone id={`month:${format(reference, 'yyyy-MM-dd')}`} className="period-plan" label="本月计划"><div className="period-plan__heading"><strong>本月计划</strong><small>拖到这里，仅安排到本月</small></div><div className="period-plan__items">{monthItems.map((item) => <TaskChip key={item.id} item={item} />)}</div><button className="period-plan__add" type="button" onClick={() => onAddToMonth?.(format(reference, 'yyyy-MM-dd'))}>＋ 添加</button></DropZone><div className="month-weekdays">{['一','二','三','四','五','六','日'].map((day) => <span key={day}>周{day}</span>)}</div><div className="month-grid">{days.map((day) => {
