...
export function PlannerPage() {
  ...
  const addToMonth = (day: string) => setEditorSchedule({ granularity: 'month', date: day })
  const addToWeek = (day: string) => setEditorSchedule({ granularity: 'week', date: day })
  ...
  return <div className="planner-page"><header className="section-heading"><div><p>计划</p><h1>把以后，放到看得见的地方。</h1><span>先安排到一个大概周期，再慢慢具体到某一天。</span></div><div className="view-tabs">{(['month','week','day'] as View[]).map((name) => <button className={view === name ? 'is-active' : ''} key={name} onClick={() => setView(name)}>{({ month: '月', week: '周', day: '日' })[name]}</button>)}</div></header>
    <DndContext sensors={sensors} onDragEnd={(event) => void dragEnd(event)}><div className="planner-layout"><section className="planner-board"><header className="planner-toolbar"><button aria-label="上一个周期" onClick={() => step(-1)}>‹</button><strong>{view === 'month' ? format(parseISO(date), 'yyyy年 M月') : date}</strong><button aria-label="下一个周期" onClick={() => step(1)}>›</button><button onClick={() => setDate(format(new Date(), 'yyyy-MM-dd'))}>今天</button></header>{view === 'month' && <MonthView referenceDate={date} items={items} onAdd={addToDay} onAddToMonth={addToMonth} />}{view === 'week' && <WeekView referenceDate={date} items={items} onAdd={addToDay} onAddToWeek={addToWeek} />}{view === 'day' && <DayView referenceDate={date} items={items} onAdd={addToDay} />}</section><DropZone id="unscheduled" className="backlog" label="待安排"><header><strong>待安排</strong><span>{unscheduled.length}</span></header><div className="backlog-items">{unscheduled.map((item) => <TaskChip key={item.id} item={item} />)}</div></DropZone></div></DndContext>
